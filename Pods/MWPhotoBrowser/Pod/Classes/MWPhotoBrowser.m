...
- (void)viewDidLoad {
	...
	// View
	self.view.backgroundColor = self.browserColor;
...
- (void)updateNavigation {
    ...
    if (_gridController) {
        if (_gridController.selectionMode) {
            ...
            NSString *photosText;
            photosText = self.gridTitle;
            //if (numberOfPhotos == 1) {
//                photosText = NSLocalizedString(@"dada", @"Used in the context: '1 photo'");
            //} else {
            //    photosText = NSLocalizedString(@"photos", @"Used in the context: '3 photos'");
            //}
            self.title = [NSString stringWithFormat:@"%@ (%lu)",  photosText, (unsigned long)numberOfPhotos];
        }
...
- (void)showGrid:(BOOL)animated {
    ...
    _gridController.gridColor = self.gridColor;
